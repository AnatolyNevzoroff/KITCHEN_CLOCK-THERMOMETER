...
//В ЭТОЙ ЖЕ ФУНКЦИИ ОТСЛЕЖИВАЕМ (независимо от того, какое меню активно):
...
if(!Timer_GO_Flag){symb5x7(Trend-1, 26,9);}//Печатаем тенденцию если не запущен таймер
...
//ДЕНЬ МЕСЯЦА
if(maxtemp_date>9){//Если число месяца состоит из двух знаков
symb3x5(maxtemp_date/10, 1,2);}//Первый разряд даты месяца
//else{symb3x5(11, 1,2);}//В противном случае ПРОБЕЛ
symb3x5(maxtemp_date%10, 5,2);//Второй разряд даты месяца
//ВРЕМЯ
if(maxtemp_hour>9){//Если число часов состоит из двух знаков
symb3x5(maxtemp_hour/10, 14,2);}//Первый разряд часов
//else{symb3x5(11, 14,2);}//В противном случае ПРОБЕЛ
symb3x5(maxtemp_hour%10, 18,2);//Второй разряд часов
symb3x5(10, 21,2);//Двоеточие
symb3x5(maxtemp_min/10, 24,2);//Первый разряд минут
symb3x5(maxtemp_min%10, 28,2);//Второй разряд минут


//МИНИМАЛЬНАЯ ТЕМПЕРАТУРА БЫЛА ЗАФИКСИРОВАНА:
if(mintemp_date>9){symb3x5(mintemp_date/10, 1,11);}//else{symb3x5(11, 1,11);}
symb3x5(mintemp_date%10, 5,11);
if(mintemp_hour>9){symb3x5(mintemp_hour/10, 14,11);}//else{symb3x5(11, 14,11);}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
//                      В ГЛАВНОМ МЕНЮ (ВМЕСТО СИМВОЛА ТЕНДЕНЦИИ ТЕМПЕРАТУРЫ)
/////////////////////////////////////////////////////////////////////////////////////////////////
//ОТОБРАЖАЕМ ОБРАТНЫЙ ОТСЧЁТ С УБЫВАЮЩИМИ МИНУТАМИ ИЛИ СЕКУНДАМИ 
   case 0:
if(minut!=0){//ЕСЛИ ПОСЛЕДНЯЯ МИНУТА ПРИ ОБРАТНОМ ОТСЧЁТЕ НЕ ДОСТИГНУТА
...
else{//ЕСЛИ ПОСЛЕДНЯЯ МИНУТА ПРИ ОБРАТНОМ ОТСЧЁТЕ ДОСТИГНУТА
//ДЛЯ БОЛЬШЕЙ ИНФОРМАТИВНОСТИ ПЕЧАТАЕМ ОСТАВШИЕСЯ СЕКУНДЫ ПОСЛЕДНЕЙ МИНУТЫ
//if(secund>9){//Если число секунд состоит из двух знаков, печатаем:
symb3x5(secund/10, 25,11);//ПЕРВЫЙ разряд СЕКУНД
//}else{symb3x5(11, 25,11);}//В противном случае ПРОБЕЛ
symb3x5(secund%10, 29,11);//Безусловно ВТОРОЙ разряд СЕКУНД
  }
   break;

/////////////////////////////////////////////////////////////////////////////////////////////////
//                                   В МЕНЮ ВЫБОРА ТАЙМЕРА 
/////////////////////////////////////////////////////////////////////////////////////////////////
//ОТОБРАЖАЕМ ОБРАТНЫЙ ОТСЧЁТ С УБЫВАЮЩИМИ МИНУТАМИ И СЕКУНДАМИ
   case 2:
if(minut>9){//Если число минут состоит из двух знаков, печатаем:
