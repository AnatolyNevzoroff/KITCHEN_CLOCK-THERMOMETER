...
//НАСТОЯТЕЛЬНО РЕКОМЕНДУЮ ЗАГРУЗИТЬ НАЧАЛЬНЫЕ ПАРАМЕТРЫ (в дальнейшем их можно заменить)
  //  #define ENABLE_VOL //НЕОБХОДИМО РАСКОММЕНТИРОВАТЬ ТОЛЬКО ПРИ САМОЙ ПЕРВОЙ ЗАГРУЗКЕ!

//ДЛЯ ПРОВЕРКИ РАБОТЫ ПЕРИФЕРИИ ЧАСОВ ВЫВОДИМ ЗНАЧЕНИЯ В МОНИТОР СЕРИЙНОГО ПОРТА 
  //  #define LOG_ENABLE_SERIAL //В РАБОЧЕЙ ВЕРСИИ НЕОБХОДИМО ЗАКОММЕНТИРОВАТЬ!
//Чтоб не путаться в потоке данных, можем отдельно включить или отключить вывод для:
  ...
  //  #define LOG_ENABLE_RADIO  //Данные о температуре с радиодатчика
...
#define IR_FFFFF 0xFFFFFFFF   //Команда "ПОВТОР" (при удержании любой кнопки)
...
//МАЛЕНЬКИЙ ШРИФТ 3Х5 
const uint8_t font3x5[12][5] PROGMEM ={//12 массивов по 5 элементов в каждом
...
//До получения реальной температуры устанавливаем предельно возможные значения 
int16_t MaxTemp=-990;// -99.0 минимально возможную в качестве максимальной
int16_t MinTemp= 990;// +99.0 максимально возможную в качестве минимальной
int16_t Temperature;//Температура с датчика
int16_t Tend_Temp;//Временное значение температуры для отслеживания тенденции
int16_t Fotorezist_OLD;//Значение освещённости для сравнения с текищей
...
uint8_t buf[7];//Буфер для хранения данных принятых с радиоприёмника
...
int8_t Count,LED_Intensity,LDR_Sensor,LED_Intens,LED_Intens_OLD=16;
...
pinMode(LED_INFO_PIN,OUTPUT);//Светодиод-индикатор приёма данных с радиодатчика (IR пульта)
...
matrix.setRotation(1);//Если не указан номер матрицы, значит поворачиваем все матрицы
...
LED_Intensity=EEPROM.read(5);//Яркость всех матриц заданная пользователем (0...9) попугаев

#ifdef ENABLE_VOL //РЕКОМЕНДУЕМЫЕ НАЧАЛЬНЫЕ ПАРАМЕТРЫ
Timer_1=10;Timer_1=30;Timer_1=50;SP_volume=10;Wait_AutoRet=15;LED_Intensity=0;
#endif
...
//тем самым предотвращаем обработку кодов повтора поступающих от ПДУ с очень короткой 
//паузой заданной производителем ПДУ
...
Old_sec=Tmr.sec;//Перезаписываем временное значение секунд
...
//Если передатчиков два и более, опрашивать необходимо чаще, например каждые 0,2 секунды
...
Old_min=60;Old_hour=24;//Задаём максимальные значения минут и часов
Show_Time();Show_Temperature();//Печатаем время и температуру вне зависимости от флагов
//Уменьшаем Timer_TEND_TEMP для выполнения функции Show_Trend() печати тенденции температуры
Timer_TEND_TEMP=Timer_TEND_TEMP-TEN_MIN;
...
//При нажатии на кнопку "MAX/MIN ТЕМПЕРАТУРА" запускается функция Info_Menu() отображающая
//разброс температур / дату и время события. В этом меню отслеживаем нажатие на "ОК" и время
//автовозврата.

//КНОПКОЙ "ОК" СБРАСЫВАЕМ МИНИАЛЬНОЕ И МАКСИМАЛЬНОЕ ЗНАЧЕНИЕ ТЕМПЕРАТУРЫ И 
//ПЕРЕХОДИМ В ГЛАВНОЕ МЕНЮ
if(OK_KEY==true){
//На момент нажатия кнопки и минимальная и максимальная температуры равны текущей
//Дату и время сброса устанавливаем одинаковыми для максимума и минимума температур
...
text("VALUE",1,0);//Пишем на экране "СБРОС ЗНАЧЕНИЙ"
text("RESET",1,9);
//Подаём звуковой сигнал
TimerFreeTone(TONE_PIN,1500,200,SP_volume);digitalWrite(TONE_PIN,HIGH);delay(50);
MENU=false;FL_START_MENU=true;//Обеспечиваем переход в основное меню
   OK_KEY=false;}//Конец сброса миниальных и максимальных значений температуры

FN_AutoRet();//Автовозврат в главное меню через заданный интервал
...
//За отображение меню отвечает функция Select_Timer() в самом меню осуществляем:
//Выбор таймера (одного из трёх)
//Изменение продолжительности выбранного таймера
//Кнопкой "ОК" запускаем или останавливаем таймер
//Отсчёт времени до автовозврата в главное меню
...
//FL_AutoRet=false;//Активируем если нет необходимости автоматом переходить в главное меню
...
FL_START_MENU_2=true;//Поднимаем флаг для УСТАНОВКИ НАЧАЛЬНЫХ ПАРАМЕТРОВ при настройке таймеров
...
//РАЗРЕШАЕМ:
FL_com_1=true;FL_com_2=true;//ПОЛУЧИТЬ И НАПЕЧАТАТЬ ТЕКУЩЕЕ ЗНАЧЕНИЕ ВЫБРАННОГО ТАЙМЕРА
FL_AutoRet=true;//ФУНКЦИЮ FN_AutoRet() ВОЗВРАТА В ГЛАВНОЕ МЕНЮ

//ЗАПРЕЩАЕМ:
Show_Time_Flag=false;//ПОКАЗ ВРЕМЕНИ (чтоб написать "TIMER")
...
//ПЕЧАТАЕМ ЗНАЧЕНИЕ (длительность) ВЫБРАННОГО ТАЙМЕРА 
if(FL_com_2==true){
...
//ЧАСЫ (0...23) Функция автоматического перехода на летнее и зимнее время отсутствует
  case 3: 
...
//Актуальные данные Числа Месяца, Месяца и Года необходимы для правильного отображения даты 
//наступления события максимальной и минимальной температуры
...
//ДЕНЬ НЕДЕЛИ (1...7) Можно не устанавливать, нигде кроме этого меню не отображается
  case 8: 
...
//Этого вполне достаточно для регулировки при установки часов в конкретных условиях освещённости.

//Для первоначальной настройки вашего экземпляра фоторезистора рекомендую использовать
//МОНИТОР СЕРИЙНОГО ПОРТА активировав  #define LOG_ENABLE_LDR  в начале скетча.

//Для экспериментов с уровнем освещённости именно НА ЭКРАНЕ каждую секунду раскомментируйте:
//if(SET_menu_3==0&&Show_Time_Flag){TWO_symbols(LDR_Sensor);Show_Time_Flag=false;FL_AutoRet=false;}
