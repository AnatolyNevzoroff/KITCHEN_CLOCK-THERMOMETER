...
if(ir.value!=IR_FFFFF){delay(p150);Count=false;REPEAT_COM=false;
switch (ir.value){
...
//КАЖДУЮ СЕКУНДУ ВЫПОЛНЯЕМ РЯД ФУНКЦИЙ
...
//ТАЙМЕР 1
  case 1: 
Timer_1=SetUP;TWO_symbols(Timer_1);
  break;

//ТАЙМЕР 2
  case 2: 
Timer_2=SetUP;TWO_symbols(Timer_2);
  break;

//ТАЙМЕР 3
  case 3: 
Timer_3=SetUP;TWO_symbols(Timer_3);
  break;
...
Timer_GO_Flag=!Timer_GO_Flag;//Запускаем или останавливаем таймер

if(Timer_GO_Flag==true){//Если ТАЙМЕР ЗАПУЩЕН
minut=SetUP-1;secund=0;Old_min=60;Old_hour=24;//FL_AutoRet=false;
...
//При каждом нажатии на кнопку "ОК":
