...
switch (Temperature){
  case 999 ... 100: //Для температуры в диапазоне от 99,9 до 10,0
symb3x7(3, 0,9);//Знак "+"
symb4x7(whole/10, 5,9);//Первый разряд градусов
  break;
  case  99 ...   1: //Для температуры в диапазоне от 9,9 до 0,1
matrix.fillRect(0,9, 8,15, LOW);//Пустота
symb3x7(3, 4,9);//Знак "+"
  break;
  case           0: //Для температуры 0,0
matrix.fillRect(0,9, 8,15, LOW);//Пустота
  break;
  case  -1 ... -99: //Для температуры в диапазоне от -0,1 до -9,9
matrix.fillRect(0,9, 8,15, LOW);//Пустота
symb3x7(2, 4,9);//Знак "-"
  break;
  case -100 ... -550: //Для температуры в диапазоне от -10,0 до -55,0
symb3x7(2, 0,9);//Знак "-"
symb4x7(whole/10, 5,9);//Первый разряд градусов
  break;
}//Конец манипуляций
...
void Show_MAX_MIN_Temp(){//Печатаем значения температуры из диапазона от -550 до +999
...
symb3x7(3, 13,0);//Знак "+" на позиции первого разряда градусов
  break;
  case           0: //Для температуры 0,0 Так как "0" это беззнаковая величина
matrix.fillRect(8,0, 16,7, LOW);//Затираем знак градусов и первый разряд градусов
  break;
  case  -1 ... -99: //Для температуры в диапазоне от -0,1 до -9,9
matrix.fillRect(8,0, 16,7, LOW);//Затираем знак градусов и первый разряд градусов
...
symb5x7(1, 0,9);
switch (MinTemp){
  case 999 ... 100:   symb3x7(3, 8,9);symb4x7(whole/10, 13,9);break;
  case  99 ...   1:   matrix.fillRect(8,9, 16,15, LOW);symb3x7(3, 13,9);break;
  case           0:   matrix.fillRect(8,9, 16,15, LOW);break;
  case  -1 ... -99:   matrix.fillRect(8,9, 16,15, LOW);symb3x7(2, 13,9);break;
  case -100 ... -550: symb3x7(2, 8,9);symb4x7(whole/10, 13,9);break;
}
